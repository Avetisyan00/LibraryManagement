...
public class AuthorBookDemo implements BookCommands {
            ...
            try {
                ...
                switch (command) {
                    ...
                    case DELETE_AUTHOR_BY_INDEX:
                        authorStorage.printAuthors();
                        System.out.println("Input model.Author's Index");
    ...
    private static void register() {
        ...
        if (userStorage.getUserByEmail(userData[2]) == null) {
            ...
            System.out.println("model.User has been created");
            UserStorage.add(user);
        } else {
            System.out.println("model.User with email " + userData[2] + " already exists!");
        }
    }

    private static void initData() {
        //Adding some Authors for test
        Author Stephen = new Author(registeredUser, "Stephen", "King", "StephenKing@gmail.com", "MALE");
        AuthorStorage.add(Stephen);
        Author Shield = new Author(registeredUser, "Herbert", "Shield", "HerbertSield@gmail.com", "MALE");
        AuthorStorage.add(Shield);
        Author Shakespeare = new Author(registeredUser, "Shakespeare", "", "Shakespeare@gmail.com", "MALE");
        AuthorStorage.add(Shakespeare);
        Author Maxim = new Author(registeredUser, "Maxim", "Maxim", "MaxMaximov@mail.ru", "MALE");
    ...
    private static void addAuthor() {
        System.out.println("Input model.Author's name");
        String name = scanner.nextLine();
        System.out.println("Input model.Author's surname");
        String surname = scanner.nextLine();
        System.out.println("Input model.Author's email");
        String email = scanner.nextLine();
        System.out.println("Input model.Author's gender");
        String gender = scanner.nextLine().toUpperCase(Locale.ROOT);
        Author author = new Author(registeredUser, name, surname, email, gender);
        AuthorStorage.add(author);
        System.out.println("The model.Author has been created");
    }
    ...
    private static void addBook() throws AuthorNotFoundException {
        try {
            System.out.println("Input model.Book's Title");
            String title = scanner.nextLine();
            System.out.println("Input model.Book's model.Author's Index");
            int index = Integer.parseInt(scanner.nextLine());
            System.out.println("Input model.Book's price");
            double price = Double.parseDouble(scanner.nextLine());
            System.out.println("Input model.Book's count");
            int count = Integer.parseInt(scanner.nextLine());
            System.out.println("Input model.Book's genre");
            String genre = scanner.nextLine();
            Book book = new Book(title, authorStorage.getAuthorByIndex(index), price, count, genre);
            bookStorage.add(book);
            System.out.println("The model.Book has been saved successfully!");
        } catch (IllegalArgumentException e) {
    ...
    static void login() throws AuthorNotFoundException {
        ...
        if (user == null) {
            System.out.println("model.User with this email " + emailPassword[0] + " does not exist");
        } else {
